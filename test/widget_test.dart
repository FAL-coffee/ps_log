...
void main() {
  testWidgets('記録を追加すると一覧に表示される', (WidgetTester tester) async {
    SharedPreferences.setMockInitialValues({'display_mode': 1});
    ...
    await tester.enterText(find.byKey(const Key('hallField')), 'テストホール');
    await tester.pumpAndSettle();

    await tester.enterText(find.byKey(const Key('machineField')), 'ユニコーン');
    await tester.pumpAndSettle();
    ...
    expect(find.text('収支: 500円'), findsOneWidget);
